<?php
// Si existe el serial de poste y descripcion se realiza la conexion a la bd
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    if (isset($_POST['seriePoste']) && isset($_POST['descripcion'])) {
        include 'conexion_db_reporte.php';
        $dbcon = conexion();
// con la funcion post se realiza el pedido de datos al usuario
        ...
        // si no hay reportes registrados o si el poste tiene 0 reportes
        if ($result_verificar) {
            if (pg_num_rows($result_verificar) == 0) {
                ...
                if ($result_existencia && pg_num_rows($result_existencia) > 0) {
                    ...
                    if ($result_actualizar) {
                        ...
                        // si el reporte se realiza o tiene errores
                        if ($result_insertar) {

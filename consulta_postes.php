<?php
// Si recibe una manzana desde el metodo post asociada a la bd
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Consulta (centroide) SQL para obtener el centroide de la manzana
    ...
    // Si se asocia una manzana se hace un zoom a esa manzana 
    if ($result_centroide) {
    ...
    // Consulta (interseccion) SQL para obtener los postes en la manzana
    ...
    // Si hay 1 manzana asociada se retorna la tabla de postes con id, serie y estado
    if ($result_postes) {
        if (pg_num_rows($result_postes) > 0) {
            echo "<table class='table'>
                    ...
                    <tbody>";
                    // mientras existan postes asociados a la manzana se realizara la tabla
